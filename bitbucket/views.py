...
from bitbucket.services.document_actions import (
...
from bitbucket.services.git_output import bounded_git_output, sanitize_git_output
from bitbucket.services.git_sync import (
    GitHTTPSCredential,
    RepositorySyncError,
    managed_repository_path,
    test_repository_connection,
)
from bitbucket.services.https_credentials import (
...
from bitbucket.services.pdf_pipeline_metrics import metrics_payload_for_request
from bitbucket.services.pdf_search import (
    PDFSearchHit,
    PDFSearchPage,
...
from bitbucket.services.pdf_stars import (
    PDFStarConflictError,
    PDFStarNotFoundError,
    PDFStarValidationError,
    set_document_star,
)
from bitbucket.services.people import (
...
from bitbucket.services.repository_activity import (
    repository_work_summary,
    with_repository_activity,
)
from bitbucket.services.repository_hosts import (
    add_trusted_repository_host,
    effective_repository_host_policy,
    https_origin_from_repository_url,
)
from bitbucket.services.repository_lifecycle import (
...
from bitbucket.services.repository_urls import (
    ...
    normalize_https_repository_url_for_registration,
...
def _pdf_timeline_page(page_number: object) -> tuple[Page, tuple[PDFTimelineGroup, ...]]:
    ...
    paginator = Paginator(queryset, settings.BITBUCKET_APP_PDF_PAGE_SIZE)
    try:
...
def _pdf_page_url(page_number: int) -> str:
    return f"{reverse('bitbucket:document_page')}?page={page_number}"
...
def _people_filter_navigation(
    ...
    if request is None:
        return (), reverse("bitbucket:index")
    ...
    clear_href = reverse("bitbucket:index")
    if clear_query:
...
def _search_page_url(
    ...
    *,
    ...
    return f"{reverse('bitbucket:index')}?{urlencode(pairs)}"
...
def _automatic_refresh_payload(repository: BitbucketRepository) -> dict[str, object]:
    ...
    globally_enabled = bool(getattr(settings, "BITBUCKET_APP_DAILY_REFRESH_ENABLED", True))
...
def _automation_payload(
    ...
    enabled = bool(getattr(settings, "BITBUCKET_APP_DAILY_REFRESH_ENABLED", True))
...
def _index_context(
    *,
    ...
    requested_search_query = PDFSearchQuery(page_size=settings.BITBUCKET_APP_SEARCH_PAGE_SIZE)
    ...
    if request is not None and active_section == "search":
        ...
        try:
            ...
            requested_search_query = replace(
                ...
                page_size=min(
                    requested_search_query.page_size, settings.BITBUCKET_APP_SEARCH_PAGE_SIZE
                ),
            )
    ...
    people_group_return_to = reverse("bitbucket:index")
    if request is not None and request.path == reverse("bitbucket:index"):
...
def _repository_payload(repository: BitbucketRepository) -> dict[str, object]:
    ...
    return {
        ...
        "automatic": automatic,
        "refreshUrl": reverse(
            "bitbucket:repository_refresh", kwargs={"repository_id": repository.pk}
        ),
        "exclusionUrl": reverse(
            "bitbucket:repository_exclude", kwargs={"repository_id": repository.pk}
        ),
        "removeUrl": reverse(
            "bitbucket:repository_remove", kwargs={"repository_id": repository.pk}
        ),
    }
...
def index(request: HttpRequest) -> HttpResponse:
    """Render the repository-aware Bitbucket workspace."""

    return render(request, "bitbucket/index.html", _index_context(request=request))
...
def add_people_group(request: HttpRequest) -> HttpResponse:
    ...
    try:
        ...
        return render(request, "bitbucket/index.html", context, status=400)
...
def document_page(request: HttpRequest) -> HttpResponse:
    ...
    if not _is_async_request(request):
        ...
        return render(request, "bitbucket/index.html", context)
...
def _document_action_return_url(
    ...
    *,
    ...
    if raw_return_to and len(raw_return_to) <= 8192:
        ...
        if not parsed.scheme and not parsed.netloc and parsed.path == reverse("bitbucket:index"):
    ...
    try:
        ...
        page_number = 0
    destination = (
        _pdf_page_url(page_number) if 2 <= page_number <= 1_000_000 else reverse("bitbucket:index")
    )
    if document_id is not None:
        return f"{destination}#pdf-document-{document_id}"
    return destination
...
def add_repository(request: HttpRequest) -> HttpResponse:
    ...
    try:
        ...
        normalized_urls = []
        origins_to_approve: set[str] = set()
        ...
        for line_number, value in submitted_lines:
            ...
            try:
                normalized = (
                    ...
                    if host_policy.externally_managed
                    else normalize_https_repository_url_for_registration(value)
                )
            ...
            if not host_policy.externally_managed:
                origin = https_origin_from_repository_url(normalized.remote_url)
                if not host_policy.allows_https_origin(origin.canonical_origin):
                    origins_to_approve.add(origin.canonical_origin)

        # Validate the complete batch before changing durable trust or
        # repository state. Each approved entry is one exact HTTPS origin; Git
        # workers still enforce that policy again before starting transport.
        for origin in sorted(origins_to_approve):
        ...
        return redirect(f"{reverse('bitbucket:repositories')}#bb-add-repository")
    ...
    if queued_job_ids:
        ...
        if failed_job_ids:
            ...
            messages.error(request, detail)
            return redirect(reverse("bitbucket:repositories"))
    ...
    messages.success(request, detail)
    return redirect(reverse("bitbucket:index"))
...
def _selected_removal_context(repository_ids: tuple[int, ...]) -> dict[str, object]:
    ...
    return {
        ...
        "cancel_url": reverse("bitbucket:index"),
    }
...
def _remove_selected_repositories(
    ...
    if request.POST.get("confirmed") != "yes":
        return render(request, "bitbucket/repositories_remove.html", context)
    removed_ids = []
    failures = []
    for repository_id in repository_ids:
        try:
    ...
    if failures:
        ...
        return render(request, "bitbucket/repositories_remove.html", context, status=409)
    messages.success(request, detail)
    return redirect(reverse("bitbucket:index"))
...
def remove_repository_view(request: HttpRequest, repository_id: int) -> HttpResponse:
    ...
    if request.method == "GET" or request.POST.get("confirmed") != "yes":
        return render(
            request,
            "bitbucket/repository_remove.html",
            context,
    ...
    try:
        ...
        return render(request, "bitbucket/repository_remove.html", context, status=409)
    detail = (
    ...
    return JsonResponse(
        {
            ...
            "workerLimits": {
                "git": settings.BITBUCKET_APP_MAX_REPO_WORKERS,
                "indexing": settings.PDF_MAX_EXTRACTION_WORKERS,
                "staging": 1,
                "publication": 1,
                "total": (
                    settings.BITBUCKET_APP_MAX_REPO_WORKERS
...
def _recovery_action_error(error: Exception) -> JsonResponse:
    from bitbucket.services.pdf_recovery import (
        RecoveryConflict,
        RecoveryNotFound,
...
def _indexing_log_payload(repository_id: int) -> dict[str, object]:
    from bitbucket.services.repository_worker_timing import latest_repository_pdf_run_jobs
...
def cancel_repository_indexing(request: HttpRequest, repository_id: int) -> HttpResponse:
    ...
    if request.POST.get("return_to") == "logs":
        messages.success(request, detail)
        return redirect(f"{reverse('bitbucket:index_status')}?repository={repository.pk}")
    return JsonResponse(
        {
...
def index_status(request: HttpRequest) -> HttpResponse:
    ...
    return render(
        ...
        "bitbucket/status.html",
        {
            ...
            "pdf_count": sum(repository.pdf_count for repository in repositories),
            "extraction_status": extraction_status,
            "repo_worker_limit": settings.BITBUCKET_APP_MAX_REPO_WORKERS,
            "index_worker_limit": settings.PDF_MAX_EXTRACTION_WORKERS,
