...
from bitbucket.services.pdf_indexing import cancel_repository_pdf_extractions
from bitbucket.services.repository_lock import (
    RepositoryCheckoutBusy,
    pdf_extraction_claim_lock,
    repository_checkout_lock,
)
from bitbucket.services.repository_sync import reserve_repository_write
...
def _roots() -> dict[str, Path]:
    ...
    return {
        "checkout": _safe_root(settings.BITBUCKET_APP_REPOSITORIES_ROOT),
        "snapshots": _safe_root(media / "bitbucket" / "excluded"),
        "staging": _safe_root(settings.BITBUCKET_APP_TEMP_ROOT),
    }
...
def _bounded_repository_checkout_lock(repository_id: int) -> Iterator[None]:
    ...
    wait_seconds = max(
        0.0,
        float(getattr(settings, "BITBUCKET_APP_REPOSITORY_REMOVAL_WAIT_SECONDS", 120)),
    )
...
def _delete_database_records(repository: BitbucketRepository) -> None:
    ...
    Notification.objects.filter(kind=NotificationKind.BITBUCKET_REFRESH).filter(
        Q(event_key=f"bitbucket-app-connection:{repository_id}")
        | Q(event_key__startswith=f"bitbucket-app-refresh:{repository_id}:")
    ).delete()
