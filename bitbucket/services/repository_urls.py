...
def _validate_host(hostname: str, *, enforce_host_policy: bool) -> str:
    if not enforce_host_policy:
        try:
            return normalize_repository_hostname(hostname)
        except RepositoryHostValidationError as error:
...
def _normalize_repository_url(
    ...
    *,
    ...
    if scp_match is not None:
        ...
        hostname = _validate_host(parsed.hostname, enforce_host_policy=enforce_host_policy)
        ...
        if scheme == "https" and enforce_host_policy:
            try:
...
def normalize_repository_url(value: object) -> NormalizedRepositoryURL:
    """Normalize a repository URL that must already satisfy the host policy."""

    return _normalize_repository_url(value, enforce_host_policy=True)


def normalize_https_repository_url_for_registration(value: object) -> NormalizedRepositoryURL:
    """Fully validate a local UI submission before approving its exact HTTPS origin.

    This is intentionally limited to registration. Outbound Git operations must
    continue to use :func:`normalize_repository_url`, which enforces the durable
    host policy before starting a transport.
    """

    normalized = _normalize_repository_url(value, enforce_host_policy=False)
    if urlsplit(normalized.remote_url).scheme.casefold() != "https":
        raise RepositoryURLValidationError(
            "unsupported_repository_protocol",
