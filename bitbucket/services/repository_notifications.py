...
from bitbucket.services.git_output import bounded_git_output
from bitbucket.services.pdf_extractor import PDF_EXTRACTOR_VERSION
from bitbucket.services.repository_activity import summarize_operation_activities
...
def repository_notification_statuses(*, at=None) -> dict[str, object]:
    ...
    target = reverse("bitbucket:index")
    status_target = reverse("bitbucket:index_status")
    items = []
        ...
        items.append(
            {
                ...
                "logsUrl": reverse("bitbucket:repository_logs", args=(repository["id"],)),
                "cancelIndexingUrl": reverse(
                    "bitbucket:repository_index_cancel", args=(repository["id"],)
                ),
