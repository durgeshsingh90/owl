...
from bitbucket.services.git_output import emit_git_output
...
from bitbucket.services.repository_lock import repository_checkout_lock
...
def _check_connection(
    ...
    *,
    ...
    _run_capture(
        ...
        timeout_seconds=settings.BITBUCKET_APP_CONNECTION_TIMEOUT_SECONDS,
...
def _run_capture(
    ...
    *,
    ...
    timeout_seconds = (
        settings.BITBUCKET_APP_GIT_TIMEOUT_SECONDS if timeout_seconds is None else timeout_seconds
    )
...
def _clone(
    ...
    shallow_since = timezone.now().date() - timedelta(
        days=365 * settings.BITBUCKET_APP_HISTORY_YEARS
    )
    try:
