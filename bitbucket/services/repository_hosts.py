...
Host approval is persisted independently from HTTPS credentials and performs no
DNS, HTTP, or Git work. Local repository registration may approve the exact
validated HTTPS origin before queuing transport work.
"""
...
def _trusted_host_model():
    return apps.get_model("bitbucket", "TrustedRepositoryHost")


def effective_repository_host_policy() -> EffectiveRepositoryHostPolicy:
    """Resolve exact origins approved by this local app without an environment list."""
    ...
    return EffectiveRepositoryHostPolicy(
        source="local_ui",
        externally_managed=False,
        entries=tuple(ui_entries[key] for key in sorted(ui_entries)),
    )
