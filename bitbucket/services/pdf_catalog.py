...
if TYPE_CHECKING:
    from bitbucket.services.git_activity import GitActivityBuild
...
def _run_binary_capture(
    ...
    *,
    ...
    deadline = time.monotonic() + settings.BITBUCKET_APP_GIT_TIMEOUT_SECONDS
    output = b""
    try:
        while True:
            ...
            if remaining <= 0:
                raise subprocess.TimeoutExpired(
                    list(arguments),
                    settings.BITBUCKET_APP_GIT_TIMEOUT_SECONDS,
                )
            try:
...
def _run_binary_spooled(
    ...
    *,
    ...
    operation = _git_command_operation(arguments)
    log_event(logger, logging.DEBUG, "catalog_git_command_started", operation=operation)
    spool_root = Path(settings.BITBUCKET_APP_TEMP_ROOT).resolve()
    try:
...
def _build_repository_pdf_catalog(
    ...
    *,
    ...
    from bitbucket.services.git_activity import build_repository_git_activity
...
def _publish_repository_pdf_catalog(
    ...
    *,
    ...
    if catalog.activity is not None:
        from bitbucket.services.git_activity import publish_repository_git_activity
    ...
    if resumed_paths:
        from bitbucket.services.pdf_local_policy import complete_resumed_policies
