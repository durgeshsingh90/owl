...
from bitbucket.services.repository_lock import pdf_search_index_repair_lock
from semantic_search.models import SemanticSourceType
from semantic_search.services.search import semantic_search

METADATA_FTS_TABLE = "bitbucket_pdf_metadata_fts"
PAGE_FTS_TABLE = "bitbucket_pdf_page_fts"
        ...
        with connection.cursor() as cursor:
            cursor.execute(
                ...
                (
                    METADATA_FTS_TABLE,
                    PAGE_FTS_TABLE,
                    "bitbucket_pdf_metadata_fts_insert",
                    "bitbucket_pdf_metadata_fts_delete",
                    "bitbucket_pdf_metadata_fts_update",
                    "bitbucket_pdf_repository_fts_update",
                    "bitbucket_pdf_page_fts_insert",
                    "bitbucket_pdf_page_fts_delete",
                    "bitbucket_pdf_page_fts_update",
                ),
            )
            row = cursor.fetchone()
...
def ensure_search_index_available() -> bool:
    ...
    if _search_index_available(log_missing=False):
        return True
    migration = import_module("bitbucket.migrations.0004_pdf_fts_indexes")
    started = perf_counter()
    try:
...
def _eligible_documents_cte(query: PDFSearchQuery) -> tuple[str, list[object]]:
    ...
    return (
        ...
        eligible_documents AS MATERIALIZED (
            ...
            JOIN bitbucket_bitbucketrepository AS repository
              ON repository.id = document.repository_id
            LEFT JOIN bitbucket_gitcommit AS last_commit
    ...
    for chip_index, chip in enumerate(query.chips):
        branches.append(
            ...
            SELECT {chip_index} AS chip_index,
                   ...
                   page.page_number
            FROM {PAGE_FTS_TABLE}
            JOIN bitbucket_pdftextpage AS page
