...
KEYRING_SERVICE = "owl.bitbucket-app.https"
...
class DatabaseHTTPSSecretStore(SecretStore):
    ...
    def _fernet() -> Fernet:
        digest = hashlib.sha256(
            f"owl.bitbucket-app-https-credential.v1:{settings.SECRET_KEY}".encode()
        ).digest()
...
def _configured_backend() -> str:
    return (
        str(getattr(settings, "BITBUCKET_APP_SECRET_BACKEND", settings.CONFLUENCE_SECRET_BACKEND))
        .strip()
        .casefold()
    )
