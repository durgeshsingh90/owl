...
from bitbucket.services.git_output import RepositoryGitLog, emit_git_output, flush_git_output
...
from bitbucket.services.pdf_catalog import (
...
from bitbucket.services.pdf_pipeline_runs import (
    accept_pipeline_run,
...
from bitbucket.services.repository_lock import repository_worker_wakeup_lock
...
from bitbucket.services.repository_worker_timing import (
...
STALE_JOB_AFTER = timedelta(seconds=settings.BITBUCKET_APP_REPOSITORY_JOB_LEASE_SECONDS)
...
def _daily_refresh_retry_delay() -> timedelta:
    return timedelta(seconds=settings.BITBUCKET_APP_DAILY_REFRESH_RETRY_SECONDS)


def _daily_refresh_max_retries() -> int:
    return max(0, int(settings.BITBUCKET_APP_DAILY_REFRESH_MAX_RETRIES))


def _repository_worker_max_retries() -> int:
    return max(0, int(settings.BITBUCKET_APP_REPOSITORY_WORKER_MAX_RETRIES))
...
def _automatic_refresh_notification_event_key(job: RepositorySyncJob) -> str | None:
    if job.trigger not in AUTOMATIC_JOB_TRIGGERS or job.scheduled_day is None:
        return None
    return f"bitbucket-app-refresh:{job.repository_id}:{job.scheduled_day.isoformat()}"
...
def _publish_manual_connection_status(
    ...
    if job.trigger != RepositorySyncTrigger.MANUAL:
        return
    event_key = f"bitbucket-app-connection:{job.repository_id}"
    try:
...
def _daily_refresh_local_time_label() -> str:
    hour = int(settings.BITBUCKET_APP_DAILY_REFRESH_LOCAL_HOUR)
...
def _daily_refresh_window(observed_at: datetime) -> tuple[date, datetime, datetime]:
    ...
    refresh_time = datetime_time(hour=settings.BITBUCKET_APP_DAILY_REFRESH_LOCAL_HOUR)
...
def _repository_automation_status(
    ...
    *,
    ...
    if not settings.BITBUCKET_APP_DAILY_REFRESH_ENABLED or not repository.enabled:
        detail = (
            "Automatic daily repository refresh is disabled in OWL settings."
            if not settings.BITBUCKET_APP_DAILY_REFRESH_ENABLED
...
def _reserve_queued_repository_worker_wakeups_once(
    *,
    ...
    with repository_worker_wakeup_lock(), transaction.atomic():
        ...
        available = max(
            0,
            settings.BITBUCKET_APP_MAX_REPO_WORKERS - running_jobs - recent_reservations,
        )
        for job in queued_jobs:
    ...
    if reserved_ids:
        log_event(
            ...
            limit=settings.BITBUCKET_APP_MAX_REPO_WORKERS,
        )
...
def launch_sync_worker() -> subprocess.Popen[bytes]:
    ...
    command = [
        ...
        "bitbucket_app_sync_worker",
        "--idle-timeout",
        str(settings.BITBUCKET_APP_WORKER_IDLE_SECONDS),
    ]
    close_old_connections()
    try:
