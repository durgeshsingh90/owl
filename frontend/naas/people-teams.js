"use strict";
const PEOPLE_PREFERENCES_KEY = "owl-naas-people-v1";
...
function groupPeopleByIdentity(records) {
  ...
  for (const person of records) {
    ...
    if (!group) {
      group = {...person, name: person.name.trim().replace(/\s+/g, " "), commits: 0, pdfCount: 0, repositories: new Set()};
      groups.set(key, group);
    }
    group.commits += Number(person.commits) || 0;
    group.pdfCount += Number(person.pdfCount) || 0;
    group.repositories.add(JSON.stringify([String(person.projectId), person.repo]));
  }
  return [...groups.values()].map(person => ({...person, repoCount: person.repositories.size}));
}
...
function renderCollapsedPeople(visiblePeople) {
  ...
  document.querySelector("#people-bar-favourite-list").innerHTML = visiblePeople.map(person => {
    const key = personKey(person);
    return `<button class="people-bar-person" type="button" data-team-filter="person:${escapeHtml(key)}" aria-pressed="${activePeopleFilter === `person:${key}`}" title="${escapeHtml(person.name)}" aria-label="Show files by ${escapeHtml(person.name)}"><span class="bar-person-initials" aria-hidden="true">${escapeHtml(getInitials(person.name))}</span>${isPersonStarred(key) ? '<span class="bar-person-star author-star" aria-hidden="true">★</span>' : ''}</button>`;
  }).join("");
}
...
(() => {
  ...
  document.querySelector("#remove-team").addEventListener("click", () => {
    ...
    if (
      !team ||
      !window.confirm(
        `Remove team “${team.name}”? People, stars, and files will be kept.`,
      )
    )
      return;
    if (
