...
(() => {
  ...
  document.querySelector("#delete-selected-repo").onclick = async () => {
    ...
    dialog.showModal();
    try {
      const preview = await crawlJson(projectId ? "/naas/api/projects/delete-preview" : "/naas/api/repositories/delete-preview", projectId ? {project_id:projectId} : {repository_ids:ids});
      if (current !== generation) return;
      document.querySelector("#delete-repo-description").textContent = `${projectId ? "Project " + preview.project.project + ". " : ""}${preview.repositories.length} repositories: ${preview.repositories.map(repo => repo.project + "/" + repo.repo).join(", ")}. Deletes ${preview.documents} File records and ${preview.failed_documents} failed-File records. ${projectId ? "Removes the project from OWL. " : ""}Remote Bitbucket data is unchanged.`;
  ...
  form.onsubmit = async event => {
    ...
    try {
      const result = await crawlJson(projectId ? "/naas/api/projects/delete" : "/naas/api/repositories/delete", {...(projectId ? {project_id:projectId} : {repository_ids:ids}), confirmation:phrase.value});
