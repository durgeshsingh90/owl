...
(() => {
  ...
  document.querySelector("#hard-retry-open").onclick = async () => {
    ...
    dialog.showModal();
    try {
      const data = await crawlJson("/naas/api/crawl/hard-retry/preview");
      scope.textContent = `${data.server}: ${data.projects.map(p => p.project).join(", ") || "No tracked projects"}. ${data.repositories} currently saved repositories, ${data.documents} File records and ${data.failed_documents} failed-File records. All repositories in these projects will be rescanned, regardless of sidebar selection.`;
  ...
  start.onclick = async () => {
    ...
    try {
      ...
      const response = await fetch("/naas/api/crawl/hard-retry", {method:"POST", headers:{"Content-Type":"application/json"}, body:JSON.stringify({confirmation:input.value})});
