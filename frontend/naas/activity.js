(() => {
  ...
  async function load() {
    ...
    try {
      const response = await fetch(`/naas/api/activity?limit=25&offset=${offset}`, {cache: 'no-store'});
      ...
      for (const run of data.items) {
        ...
        element('p', `${count(repos.length)} repositories checked · ${count(repos.filter(r => (r.new || r.updated || r.deleted)).length)} affected · ${count(sum('new') + sum('updated') + sum('deleted'))} File changes · New ${count(sum('new'))} · Updated ${count(sum('updated'))} · Deleted ${count(sum('deleted'))} · Failed ${count(sum('failed'))}`, section);
