...
// Use the stable File URL, not a display name or temporary numeric sample ID.
function pdfNoteKey(pdf) {
  return `owl-naas-pdf-note:${pdf.pdfUrl}`;
}
...
(() => {
  ...
  document
    ...
    .addEventListener("submit", async (event) => {
      ...
      if (!target) return;
      try {
        const response = await fetch(`/naas/api/document/${target.id}/notes`, {
          method: "PATCH",
  ...
  window.addEventListener("storage", (event) => {
    if (event.key === null || event.key.startsWith("owl-naas-pdf-note:"))
      renderPdfTable();
  });
})();
