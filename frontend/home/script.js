...
(async () => {
  ...
  const metrics = [
    ...
    [
      ...
      "Recorded document opens",
    ],
    [
      "New PDFs this week",
      "—",
      "Awaiting document-added dates · Monday–Sunday",
    ],
    ["New PDFs in period", "—", "Awaiting document-added dates"],
    ["Updated PDFs in period", "—", "Awaiting document update history"],
    ["Inactive repositories", inactiveCount, "No commits for over 3 months"],
  ];
  ...
  document.querySelector("#insight-title").textContent =
    "Across saved repositories";
  document.querySelector("#insight-copy").textContent = total
    ? `${projectTotals[0].name} has ${projectTotals[0].total} saved PDFs.`
    : "No PDFs have been indexed yet.";
  document.querySelector("#project-bars").innerHTML = projectTotals
    .map(
      (project) =>
        `<div class="bar-row"><div class="bar-title"><span>${escape(project.name)}</span><strong>${project.total}</strong></div><div class="track" role="img" aria-label="${escape(project.name)}: ${project.total} PDFs"><span style="width:${total ? (project.total / total) * 100 : 0}%"></span></div></div>`,
    )
    .join("");
  ...
  function renderActivity() {
    ...
    document.querySelector("#contributors").innerHTML = filtered
      ...
      : activeRange.all
        ? [...people]
            ...
            .slice(0, 20)
            .map((person) =>
              row(person.name, person.repo, `${person.commits} commits`),
            )
            .join("")
        : empty(
            "Monthly contributor activity requires timestamped commits. Choose All time to see available totals.",
          );
    ...
    document.querySelector("#popular").innerHTML = ranked
      ...
      : empty(
          "Dated open events are not connected. Choose All time to see the top 20 PDFs by recorded opens.",
        );
    ...
    if (Array.isArray(overviewData.documentEvents)) {
      ...
      setMetric(
        ...
        unique(
          "added",
          (at) => eventDay(at) >= monday && eventDay(at) <= today,
        ),
  ...
  document.querySelector("#inactive-repos").innerHTML =
    repos
      ...
      .join("") ||
    empty("No repositories have been inactive for more than three months.");
  applyPeriod();

  Object.assign(window, { number, escape, row, empty });
  Object.defineProperty(window, "activeRange", { get: () => activeRange });
