...
| PIPE-007 | The canonical Settings page keeps Overview, Confluence, Repository sources, and Bookmark data as separate addressable sections; detailed pipeline diagnostics remain in Repository Logs. A trusted credential-free host origin, a credential origin, and a full repository clone URL remain distinct, with external policy precedence and dependency-safe removal. |
...
| PIPE-009 | Controller modes are `fixed`, `observe`, `shadow`, and explicitly enabled `adaptive`, with one supervisor-owned authority. Requested, configured, tested, resource-aware, safety, and effective targets remain distinct; the initial tested PDF ceiling is eight. |
