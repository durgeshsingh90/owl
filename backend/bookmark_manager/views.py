...
def _breadcrumb_payload(items: Iterable[BreadcrumbItem]) -> list[dict[str, object]]:
    return [{"title": item.title, "url": item.url, "isLeaf": item.is_leaf} for item in items]


def _bookmark_payload(bookmark: Bookmark, *, breadcrumb=()) -> dict[str, object]:
    """Serialize only display metadata and server-validated action URLs."""

    recency = bookmark.recency_status
    category = bookmark.category
    manual_folder = bookmark.manual_folder
    return {
        "id": bookmark.pk,
        "pageId": bookmark.page_id,
        "outlineNumber": getattr(bookmark, "outline_number", str(bookmark.pk)),
        "title": bookmark.title,
        "url": bookmark.url,
        "sourceType": bookmark.source_type,
        "sourceLabel": bookmark.get_source_type_display(),
        "spaceName": bookmark.space_name,
        "spaceKey": bookmark.space_key,
        "version": bookmark.version,
        "createdAt": _iso(bookmark.created_at),
        "updatedAt": _iso(bookmark.updated_at),
        "savedAt": _iso(bookmark.saved_at),
        "createdBy": bookmark.created_by_name,
        "modifiedBy": bookmark.modified_by_name,
        "author": bookmark.author_name,
        "category": (
            {"id": category.pk, "name": category.name, "domain": category.domain}
            if category is not None
            else None
        ),
        "manualFolder": (
            {"id": manual_folder.pk, "name": manual_folder.name}
            if manual_folder is not None
            else None
        ),
        "pageTextSizeBytes": bookmark.page_text_size_bytes,
        "favorite": bookmark.favorite,
        "pinned": bookmark.pinned,
        "tags": [tag.name for tag in bookmark.tags.all()],
        "notes": bookmark.notes,
        "notesUpdatedAt": _iso(bookmark.notes_updated_at),
        "openCount": bookmark.open_count,
        "firstOpenedAt": _iso(bookmark.first_opened_at),
        "lastViewedAt": _iso(bookmark.last_viewed_at),
        "lastViewedVersion": bookmark.last_viewed_version,
        "changedSinceViewed": bookmark.changed_since_viewed,
        "lastRefreshAttemptAt": _iso(bookmark.last_refresh_attempt_at),
        "lastRefreshedAt": _iso(bookmark.last_refreshed_at),
        "availability": bookmark.availability_status,
        "availabilityLabel": bookmark.get_availability_status_display(),
        "recency": str(recency),
        "recencyLabel": recency.label,
        "breadcrumb": _breadcrumb_payload(breadcrumb),
        "selectUrl": f"{reverse('bookmark_manager:index')}?{urlencode({'selected': bookmark.pk})}",
        "openUrl": reverse("bookmark_manager:open", args=(bookmark.pk,)),
        "openParentUrl": reverse("bookmark_manager:open_parent", args=(bookmark.pk,)),
        "organiseUrl": reverse("bookmark_manager:organise", args=(bookmark.pk,)),
        "favoriteUrl": reverse("bookmark_manager:favorite", args=(bookmark.pk,)),
        "pinUrl": reverse("bookmark_manager:pin", args=(bookmark.pk,)),
        "deleteUrl": reverse("bookmark_manager:delete", args=(bookmark.pk,)),
    }


def _tree_item_payload(item: BookmarkTreeItem) -> dict[str, object]:
    bookmark = item.bookmark
    return {
        "id": item.node.pk,
        "title": item.node.title,
        "outlineNumber": item.outline_number,
        "depth": item.depth,
        "matches": item.matches,
        "selected": item.selected,
        "located": item.located,
        "openCount": item.subtree_open_count,
        "bookmark": _bookmark_payload(bookmark) if bookmark is not None else None,
        "children": [_tree_item_payload(child) for child in item.children],
    }


def _bookmark_workspace_payload(request: HttpRequest) -> dict[str, object]:
    context = _index_context(request)
    query_result = context["query_result"]
    timeline_page = context["bookmark_timeline_page"]
    selected = context["selected_bookmark"]
    timeline_query = context["timeline_paging_query_string"]

    def timeline_page_url(page_number: int) -> str:
        query = f"{timeline_query}&" if timeline_query else ""
        return f"{reverse('bookmark_manager:index')}?{query}timeline_page={page_number}"

    return {
        "ok": True,
        "csrfToken": get_token(request),
        "statusMessage": context["status_message"],
        "inlineError": context["inline_error"],
        "urls": {
            "home": reverse("core:dashboard"),
            "index": reverse("bookmark_manager:index"),
            "workspace": reverse("bookmark_manager:workspace"),
            "save": reverse("bookmark_manager:save"),
            "settings": reverse("bookmark_manager:settings"),
            "systemStatus": reverse("core:system_status"),
            "bitbucket": reverse("bitbucket:index"),
            "refreshStart": reverse("bookmark_manager:refresh_start"),
            "refreshStatus": reverse("bookmark_manager:refresh_status"),
            "connectionTest": reverse("bookmark_manager:saved_connection_test"),
            "deleteSelected": reverse("bookmark_manager:delete_selected"),
            "folderCreate": reverse("bookmark_manager:folder_create"),
            "folderMove": reverse("bookmark_manager:folder_move"),
        },
        "search": {
            "term": context["search_term"],
            "urlSearch": context["url_search_active"],
            "urlMatchCount": context["url_identity_match_count"],
            "firstUrlMatch": context["first_url_identity_match_href"],
            "semanticFallback": query_result.semantic_fallback_used,
        },
        "resultCount": context["result_count"],
        "totalBookmarks": context["total_bookmarks"],
        "counts": context["bookmark_counts"],
        "activeFilters": [
            {"label": item.label, "value": item.value} for item in context["active_filters"]
        ],
        "sortControls": [
            {
                "key": item.key,
            ...
            for item in context["sort_controls"]
        ],
        "refresh": context["refresh_status"],
        "tree": [_tree_item_payload(item) for item in context["tree_items"]],
        "manualFolders": [
            {
                "id": group.folder.pk,
                "name": group.folder.name,
                "bookmarkCount": group.bookmark_count,
                "openCount": group.open_count,
                "items": [_tree_item_payload(item) for item in group.items],
            }
            for group in context["manual_folder_items"]
        ],
        "folders": [
            {"id": folder.pk, "name": folder.name} for folder in context["bookmark_folders"]
        ],
        "flatItems": [
            _bookmark_payload(item.bookmark, breadcrumb=item.breadcrumb)
            for item in context["flat_items"]
        ],
        "selectedBookmark": (
            _bookmark_payload(selected, breadcrumb=context["selected_breadcrumb"])
            if selected is not None
            else None
        ),
        "similarBookmarks": [
            _bookmark_payload(bookmark) for bookmark in context["similar_bookmarks"]
        ],
        "timeline": [
            {
                "key": group.key,
                "label": group.label,
                "bookmarks": [_bookmark_payload(bookmark) for bookmark in group.bookmarks],
            }
            for group in timeline_page.groups
        ],
        "timelinePagination": {
            "current": timeline_page.number,
            "total": timeline_page.page_count,
            "totalCount": timeline_page.total_count,
            "firstItem": timeline_page.first_item_number,
            "lastItem": timeline_page.last_item_number,
            "previousUrl": (
                timeline_page_url(timeline_page.number - 1) if timeline_page.has_previous else None
            ),
            "nextUrl": (
                timeline_page_url(timeline_page.number + 1) if timeline_page.has_next else None
            ),
        },
        "people": [
            {
                "name": contact.name,
                "pageCount": contact.page_count,
                "writtenCount": contact.written_count,
                "updatedCount": contact.updated_count,
                "selected": contact.selected,
            }
            for contact in context["confluence_contacts"]
        ],
        "categories": [
            {
                "id": category.pk,
                ...
                "bookmarkCount": category.total_bookmarks,
            }
            for category in context["bookmark_categories"]
        ],
        "tagSuggestions": list(context["tag_suggestions"]),
    }
...
def index(request: HttpRequest) -> HttpResponse:
    """Render the minimal document that mounts the React Bookmark Manager."""
...
def workspace(request: HttpRequest) -> JsonResponse:
    """Return the Bookmark Manager's current query and safe action contract."""

    response = JsonResponse(_bookmark_workspace_payload(request))
...
def _settings_workspace_payload(request: HttpRequest) -> dict[str, object]:
    context = _settings_page_context(request)
    configuration = context["configuration"]
    settings_form = context["settings_form"]
    import_run = context["last_import_run"]
    return {
        "ok": True,
        "csrfToken": get_token(request),
        "statusMessage": context["status_message"],
        "notice": context["settings_notice"],
        "selectedSection": context["selected_settings_section"],
        "selectedTask": context["settings_task"],
        "sections": list(context["settings_sections"]),
        "urls": {
            "home": reverse("core:dashboard"),
            "bookmarks": reverse("bookmark_manager:index"),
            "settings": reverse("bookmark_manager:settings"),
            "workspace": reverse("bookmark_manager:settings_workspace"),
            "systemStatus": reverse("core:system_status"),
            "confluenceTest": reverse("bookmark_manager:settings_test"),
            "confluenceSave": reverse("bookmark_manager:settings_save"),
            "confluenceRemove": reverse("bookmark_manager:settings_remove"),
            "bookmarkImport": reverse("bookmark_manager:import"),
            "bookmarkExport": reverse("bookmark_manager:export"),
        },
        "configuration": {
            **asdict(configuration),
            "last_verified_at": _iso(configuration.last_verified_at),
            "baseUrl": settings_form["base_url"].value() or "",
            "authMode": settings_form["auth_mode"].value() or "bearer",
        },
        "bookmarkCounts": context["bookmark_counts"],
        "lastImport": (
            {
                "id": import_run.pk,
                "status": import_run.get_status_display(),
                "outcome": import_run.outcome,
                "imported": import_run.imported_records,
                "skipped": import_run.skipped_records,
                "failed": import_run.failed_records,
                "completedAt": _iso(import_run.completed_at),
            }
...
@never_cache
@_logged_view
def settings_workspace(request: HttpRequest) -> JsonResponse:
    """Return secret-free configuration summaries for the React Settings UI."""

    response = JsonResponse(_settings_workspace_payload(request))
    response.headers["Cache-Control"] = "no-store"
    return response
...
def save_settings(request: HttpRequest) -> HttpResponse:
    ...
    if _is_async_form(request):
        return JsonResponse(
            {
                "state": "success",
                "label": "Confluence settings saved",
                "detail": result.detail,
                "redirect": _settings_url(section="confluence", action="confluence-saved"),
            }
        )
...
def remove_settings(request: HttpRequest) -> HttpResponse:
    ...
    if _is_async_form(request):
        return JsonResponse(
            {
                "state": "success",
                "label": "Confluence connection removed",
                "detail": result.detail,
                "redirect": _settings_url(section="confluence", action="confluence-removed"),
            }
        )
