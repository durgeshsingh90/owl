...
def path_problems(path: PurePosixPath, size: int) -> list[str]:
    ...
    top_level_parts = path.parts[:1]
    if top_level_parts and top_level_parts[0].lower() == "backend":
        top_level_parts = path.parts[1:2]
    if top_level_parts and top_level_parts[0].lower() in BLOCKED_TOP_LEVEL_DIRECTORIES:
...
def content_problems(text: str, *, path: PurePosixPath | None = None) -> list[tuple[int, str]]:
    ...
    javascript_source = path is not None and path.suffix.casefold() in {
        ".cjs",
        ".js",
        ".jsx",
        ".mjs",
