"""Queue each repository's daily API refresh once in OWL's local timezone."""
...
def queue_due_daily_refreshes(*, at: datetime | None = None) -> tuple[SyncJob, ...]:
    if not getattr(settings, "BITBUCKET_APP_DAILY_REFRESH_ENABLED", True):
        return ()
    observed_at = at or timezone.now()
    local_time = timezone.localtime(observed_at)
    configured_hour = int(getattr(settings, "BITBUCKET_APP_DAILY_REFRESH_LOCAL_HOUR", 9))
    ...
    ).exclude(last_successful_refresh_on=scheduled_day)
    for repository in repositories.iterator():
        ...
        if created:
            Repository.objects.filter(pk=repository.pk).update(
                ...
                status_message="Today's metadata refresh is queued.",
            )
            jobs.append(job)
    return tuple(jobs)
    ...
    if job.status in {
        ...
        Repository.objects.filter(pk=job.repository_id).update(
            state=(
                ...
                if job.operation == SyncOperation.INITIAL
