...
@pytest.mark.parametrize(
    ...
    [
        ...
        "backend/var/database/owl.sqlite3",
    ],
)
...
def test_typescript_string_type_is_not_mistaken_for_an_internal_hostname(suffix: str):
    source = "requestUrl: string,\nrepositoryHost: string;"

    assert scanner.content_problems(source, path=PurePosixPath(f"types{suffix}")) == []
