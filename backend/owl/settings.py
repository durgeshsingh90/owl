...
# Independent Bitbucket Data Center crawler. Credentials and repository URLs are
# saved through the app; all operational tuning lives here rather than in a
# separate crawler INI file.
BITBUCKET_APP_VERIFY_SSL = _env_bool("BITBUCKET_APP_VERIFY_SSL", True)
BITBUCKET_APP_API_TIMEOUT_SECONDS = min(
    _env_int("BITBUCKET_APP_API_TIMEOUT_SECONDS", 30, minimum=1),
    120,
)
BITBUCKET_APP_API_PAGE_SIZE = min(
    _env_int("BITBUCKET_APP_API_PAGE_SIZE", 1000, minimum=25),
    1000,
)
...
BITBUCKET_APP_MAX_WORKERS = min(
    _env_int("BITBUCKET_APP_MAX_WORKERS", 1, minimum=1),
    16,
)
BITBUCKET_APP_WORKER_IDLE_SECONDS = min(
    _env_int("BITBUCKET_APP_WORKER_IDLE_SECONDS", 30, minimum=1),
    300,
)
BITBUCKET_APP_PDF_DOWNLOAD_TIMEOUT_SECONDS = min(
    _env_int("BITBUCKET_APP_PDF_DOWNLOAD_TIMEOUT_SECONDS", 120, minimum=1),
    600,
)
BITBUCKET_APP_PDF_DOWNLOAD_RETRIES = min(
    _env_int("BITBUCKET_APP_PDF_DOWNLOAD_RETRIES", 4, minimum=0),
    10,
)
BITBUCKET_APP_PDF_RETRY_BASE_SECONDS = min(
    _env_int("BITBUCKET_APP_PDF_RETRY_BASE_SECONDS", 10, minimum=0),
    300,
)
BITBUCKET_APP_PDF_RETRY_MAX_SECONDS = min(
    _env_int("BITBUCKET_APP_PDF_RETRY_MAX_SECONDS", 60, minimum=0),
    600,
)
BITBUCKET_APP_PDF_MAX_BYTES = min(
    _env_int("BITBUCKET_APP_PDF_MAX_BYTES", 104_857_600, minimum=1_048_576),
    1_073_741_824,
)
BITBUCKET_APP_PDF_MAX_PAGES = min(
    _env_int("BITBUCKET_APP_PDF_MAX_PAGES", 25_000, minimum=1),
    100_000,
)
BITBUCKET_APP_PDF_MAX_TEXT_CHARACTERS = min(
    _env_int("BITBUCKET_APP_PDF_MAX_TEXT_CHARACTERS", 50_000_000, minimum=10_000),
    250_000_000,
)
BITBUCKET_APP_SEARCH_QUERY_MAX_CHARACTERS = min(
    _env_int("BITBUCKET_APP_SEARCH_QUERY_MAX_CHARACTERS", 200, minimum=10),
    1_000,
)
...
BITBUCKET_APP_DAILY_REFRESH_LOCAL_HOUR = _env_int(
    "BITBUCKET_APP_DAILY_REFRESH_LOCAL_HOUR",
    9,
    minimum=0,
)
...
LOGGING = {
    ...
    "filters": {
        ...
        "routine_bitbucket_requests": {
            "()": "core.logging.RoutineBitbucketRequestFilter",
        },
    },
    "formatters": {
        "safe": {
    ...
    "handlers": {
        "console": {
            ...
            "formatter": "safe",
            "filters": ["expected_loopback_disconnect", "routine_bitbucket_requests"],
        },
        "local_file": {
