...
def test_date_window_includes_both_bounds_and_excludes_old_future_and_provenance_only():
    repository = _repository()
    for offset in (timedelta(), timedelta(days=-7)):
        _commit(repository, at=NOW + offset, folders=("included",))
    _commit(repository, at=NOW - timedelta(days=7, microseconds=1), folders=("old",))
...
@pytest.mark.parametrize(
    ("period", "at", "expected"),
    [
        ("week", (2026, 8, 31), (2026, 8, 24)),
        ("month", (2026, 3, 31), (2026, 2, 28)),
        ("month", (2024, 3, 31), (2024, 2, 29)),
        ("six_months", (2026, 8, 31), (2026, 2, 28)),
        ("six_months", (2026, 1, 31), (2025, 7, 31)),
        ("year", (2024, 2, 29), (2023, 2, 28)),
    ],
)
def test_periods_use_rolling_calendar_intervals_and_clamp_month_ends(period, at, expected):
    ...
    assert dashboard.started_at == datetime(*expected, 11, 30, tzinfo=UTC)
...
def test_calendar_windows_use_application_timezone_across_daylight_saving():
    ...
    included = datetime(2026, 2, 28, 11, 30, tzinfo=UTC)  # 11:30 local before DST.
...
def test_dashboard_uses_six_read_only_queries_independent_of_repository_count(
    ...
    for index in range(30):
        ...
        for person in ("Shared person", f"Person {index:02}"):
            _commit(repository, person, folders=("src", "docs"))

    with CaptureQueriesContext(connection) as queries, django_assert_num_queries(6):
        dashboard = get_bitbucket_dashboard(now=NOW)
