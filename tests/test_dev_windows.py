...
spec = importlib.util.spec_from_file_location('owl_dev',Path(__file__).resolve().parents[1]/'dev.py')
...
class WindowsLauncherTests(unittest.TestCase):
    ...
    def test_identity_requires_path_and_token(self):
        ...
        with patch.object(dev,'WINDOWS',True),patch.object(dev.subprocess,'run') as run:
            run.return_value=subprocess.CompletedProcess([],0,stdout=f'python "{dev.ROOT / "dev.py"}" _serve --token unique-test-token')
    ...
    def test_windows_stop_uses_request_file(self):
        with tempfile.TemporaryDirectory() as directory,patch.object(dev,'WINDOWS',True),patch.object(dev,'RUNTIME',Path(directory)),patch.object(dev,'STATE',Path(directory)/'state.json'),patch.object(dev,'owned',side_effect=[True,False,False]):
