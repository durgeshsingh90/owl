...
class SemanticSourceType(models.TextChoices):
    """Canonical content families supported by the shared semantic index."""

    PDF_REVISION = "pdf_revision", "PDF revision"
...
class SemanticIndex(models.Model):
    ...
    pdf_revision = models.OneToOneField(
        "bitbucket_search.PDFTextRevision",
    ...
    def __str__(self) -> str:
        source_id = self.pdf_revision_id or self.bookmark_id
        return f"{self.get_source_type_display()} {source_id} — {self.model_version}"
...
class SemanticIndexJob(models.Model):
    ...
    source_type = models.CharField(max_length=16, choices=SemanticSourceType, db_index=True)
    pdf_revision = models.ForeignKey(
        "bitbucket_search.PDFTextRevision",
        on_delete=models.CASCADE,
        null=True,
        blank=True,
    ...
    def __str__(self) -> str:
        source_id = self.pdf_revision_id or self.bookmark_id
        return f"{self.get_source_type_display()} {source_id} — {self.get_status_display()}"
